...
namespace pEngine::girEngine::backend::vulkan {
    VulkanBackend::VulkanBackend(VulkanBackend &other)
        : allocator(other.allocator),
          threadPoolHandle(other.threadPoolHandle),
          applicationContext(other.applicationContext),
          renderer(std::move(other.renderer)),
          presentationEngine(std::move(other.presentationEngine)),
          cameraUniformBufferUid(other.cameraUniformBufferUid),
          lightingUniformBufferUid(other.lightingUniformBufferUid),
          imageAcquiredSemaphore(other.imageAcquiredSemaphore),
          renderCompleteSemaphore(other.renderCompleteSemaphore),
          resourceRepository(other.resourceRepository),
          vulkanShaderModules(other.vulkanShaderModules),
          vulkanPrograms(std::move(other.vulkanPrograms)),
          vulkanGraphicsPipelines(std::move(other.vulkanGraphicsPipelines)),
          vulkanRenderPasses(other.vulkanRenderPasses) {
    }

    VulkanBackend::VulkanBackend(VulkanBackend &&other) noexcept
        : allocator(other.allocator),
          threadPoolHandle(std::move(other.threadPoolHandle)),
          applicationContext(std::move(other.applicationContext)),
          renderer(std::move(other.renderer)),
          presentationEngine(std::move(other.presentationEngine)),
          cameraUniformBufferUid(std::move(other.cameraUniformBufferUid)),
          lightingUniformBufferUid(std::move(other.lightingUniformBufferUid)),
          imageAcquiredSemaphore(other.imageAcquiredSemaphore),
          renderCompleteSemaphore(other.renderCompleteSemaphore),
          resourceRepository(std::move(other.resourceRepository)),
          vulkanShaderModules(std::move(other.vulkanShaderModules)),
          vulkanPrograms(std::move(other.vulkanPrograms)),
          vulkanGraphicsPipelines(std::move(other.vulkanGraphicsPipelines)),
          vulkanRenderPasses(std::move(other.vulkanRenderPasses)) {
    }
    ...
    VulkanBackend::VulkanBackend(const CreationInput &createInfo) : allocator(VK_NULL_HANDLE),
        ...
        applicationContext = std::make_shared<vulkan::VulkanApplicationContext>(
            vulkan::VulkanApplicationContext::CreationInput{
    ...
    void VulkanBackend::bakeVulkanDynamicRenderPass(DynamicRenderPassIR &renderPass) {
        ...
        // ALSO TODO -> move program creation outside of the pass
    ...
    VulkanBackend::createGraphicsPipelineForDynamicRenderPass(const RenderPassIR &renderPass,
                                                              ...
                                                              newPassProgram) {
        const std::vector<VkVertexInputAttributeDescription> vertexAttribs = newPassProgram->
    ...
    Backend::ConsumeResult
    VulkanBackend::consumeGirs(const frontend::BakeOutput &girList) {
    ...
    Backend::DrawFrameResult VulkanBackend::drawFrame() {
    ...
    VulkanBackend::getPipelineShaderStages(const RenderPassIR *renderPassIr) const {
        ...
        // todo -> support custom shader stage bit, in case you want to force a particular setting
    ...
    VulkanRenderer &VulkanBackend::getRenderer() {
        return *renderer;
    }

    VkPipelineRenderingCreateInfo
    VulkanBackend::getPipelineRenderingCreateInfo(const RenderPassIR *pass) {
