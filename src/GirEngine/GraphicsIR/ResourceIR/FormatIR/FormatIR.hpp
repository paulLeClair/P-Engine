...
namespace pEngine::girEngine::gir::resource {

    /**
     * I'm still kinda puzzled on the best way to handle formats in a generic way
     * without becoming mentally unstable.
     *
     * For the time being, this will just shadow the VkFormat specification 1-1
     * which should be fine and dandy for the current state of the engine.
     */
    enum class FormatIR {
        UNDEFINED = 0,
        SWAPCHAIN_DEPENDENT = -1, // placeholder til i figure out how best to make swapchain image params configurable (not that impt for now)
        R4G4_UNORM_PACK8 = 1,
