...
namespace pEngine::girEngine::scene {
    ...
    public:
        ...
        explicit Buffer(const CreationInput &createInfo)
            ...
            bufferData = ByteArray(ByteArray::CreationInput{
                createInfo.name,
                createInfo.uid,
        ...
        template<typename RawDataType>
        [[nodiscard]] const std::vector<RawDataType> &getBufferDataAsArray() const {
        ...
        [[nodiscard]] const ByteArray &getRawDataContainer() const {
            return bufferData;
        }
