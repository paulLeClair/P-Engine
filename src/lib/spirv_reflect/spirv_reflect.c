    ...
    for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count; ++descriptor_index) {
        ...
        if (p_descriptor->uav_counter_id != UINT32_MAX) {
            for (uint32_t counter_descriptor_index = 0;
                 counter_descriptor_index < p_module->descriptor_binding_count; ++counter_descriptor_index) {
    ...
    for (uint32_t member_index = 0; member_index < p_var->member_count; ++member_index) {
        ...
        switch (p_member_type->op) {
            ...
            case SpvOpTypeArray: {
                ...
                bool is_struct =
                        (p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_STRUCT) == SPV_REFLECT_TYPE_FLAG_STRUCT;
                if (is_struct) {
                    SpvReflectResult result = ParseDescriptorBlockVariableSizes(p_parser, p_module, false, true,
            ...
            case SpvOpTypeRuntimeArray: {
                bool is_struct =
                        (p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_STRUCT) == SPV_REFLECT_TYPE_FLAG_STRUCT;
                if (is_struct) {
                    SpvReflectResult result = ParseDescriptorBlockVariableSizes(p_parser, p_module, false, true, true,
                                                                                p_member_var);
                    if (result != SPV_REFLECT_RESULT_SUCCESS) {
                        return result;
                    }
...
static SpvReflectResult ParseInterfaceVariable(
    ...
    if (p_type->member_count > 0) {
        ...
        for (uint32_t member_index = 0; member_index < p_type_node->member_count; ++member_index) {
            ...
            SpvReflectResult result = ParseInterfaceVariable(p_parser, NULL, p_member_decorations, p_module,
...
static SpvReflectResult ParseInterfaceVariables(
    ...
    if (p_entry->interface_variable_count > 0) {
        p_entry->interface_variables = (SpvReflectInterfaceVariable *) calloc(p_entry->interface_variable_count,
...
static SpvReflectResult ParsePushConstantBlocks(
    ...
    p_module->push_constant_blocks = (SpvReflectBlockVariable *) calloc(p_module->push_constant_block_count,
