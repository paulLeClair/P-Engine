...
#include "../VulkanGuiThread/VulkanGuiContext.hpp"
#include "Frame.hpp"
...
vulkan::VulkanRenderer::VulkanRenderer(const VulkanRenderer &other)
    : guiThreadIsEnabled(other.guiThreadIsEnabled),
      frameCount(other.frameCount),
      applicationContext(other.applicationContext),
      presentationEngine(other.presentationEngine),
      renderPasses(other.renderPasses),
      perPassCopySemaphores(other.perPassCopySemaphores),
      allocator(other.allocator),
      imageAcquiredSemaphores(other.imageAcquiredSemaphores),
      currentImageAcquiredSemaphore(other.currentImageAcquiredSemaphore),
      commandPool(other.commandPool),
      clearColor(other.clearColor),
      guiThread(std::make_unique<VulkanGuiContext>(
          *this,
          *applicationContext,
          presentationEngine,
          other.guiThread->getCallbacks()
      )),
      previousFrameIndex(other.previousFrameIndex),
    ...
    for (auto &f: other.frames) {
        frames.emplace_back(std::make_unique<Frame>(
            ...
            f->fence,
            f->swapchainImage,
            f->imageAcquiredSemaphore,
            f->renderCompleteSemaphore,
            f->swapchainImageViews,
            f->commandPool,
            f->allocatedCommandBuffers
        ));
    }
}

vulkan::VulkanRenderer &vulkan::VulkanRenderer::operator=(const VulkanRenderer &other) {
    if (this == &other)
        return *this;
    guiThreadIsEnabled = other.guiThreadIsEnabled;
    frameCount = other.frameCount;
...
vulkan::VulkanRenderer &vulkan::VulkanRenderer::operator=(VulkanRenderer &other) {
    if (this == &other)
        return *this;
    guiThreadIsEnabled = other.guiThreadIsEnabled;
    frameCount = other.frameCount;
    ...
    commandPool = other.commandPool;
    clearColor = other.clearColor;
    std::swap(guiThread, other.guiThread);
    previousFrameIndex = other.previousFrameIndex;
    currentFrameIndex = other.currentFrameIndex;
    return *this;
}

vulkan::VulkanRenderer &vulkan::VulkanRenderer::operator=(VulkanRenderer &&other) noexcept {
    if (this == &other)
        return *this;
    guiThreadIsEnabled = other.guiThreadIsEnabled;
    frameCount = other.frameCount;
    frames = std::move(other.frames);
    applicationContext = std::move(other.applicationContext);
    presentationEngine = other.presentationEngine;
    renderPasses = std::move(other.renderPasses);
    perPassCopySemaphores = std::move(other.perPassCopySemaphores);
    allocator = other.allocator;
    imageAcquiredSemaphores = std::move(other.imageAcquiredSemaphores);
    currentImageAcquiredSemaphore = other.currentImageAcquiredSemaphore;
    commandPool = other.commandPool;
    clearColor = std::move(other.clearColor);
    guiThread = std::move(other.guiThread);
    previousFrameIndex = other.previousFrameIndex;
    currentFrameIndex = other.currentFrameIndex;
    return *this;
}
...
vulkan::VulkanRenderer::VulkanRenderer(const CreationInput &createInfo)
    ...
    guiThread = std::make_unique<VulkanGuiContext>(std::move(
            VulkanGuiContext{
                *this,
                *applicationContext,
                presentationEngine,
...
void vulkan::VulkanRenderer::newImguiFrame() const {
    if (guiThreadIsEnabled) {
        guiThread->beginNewImguiFrame();
    }
}

void vulkan::VulkanRenderer::setGuiCallbacks(std::vector<std::function<void()> > &&callbacks) {
    if (!guiThreadIsEnabled) {
        // TODO -> log!
        return;
    }
...
void vulkan::VulkanRenderer::freePreviouslyUsedCommandBuffers(const VkCommandPool commandPool) {
    const std::vector<VkCommandBuffer> allocatedFrameCommandBuffers
            = frames[currentFrameIndex]->allocatedCommandBuffers;
...
std::vector<VkSemaphore> vulkan::VulkanRenderer::constructAndSubmitRenderCommands() {
    ...
    std::vector<VkCommandBufferSubmitInfo> frameInitCommandBufferSubmissions = {};
    ...
    for (auto &buffer: renderPassCopyCommands) {
        frameInitCommandBufferSubmissions.push_back({
            .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_SUBMIT_INFO,
            .commandBuffer = buffer,
        });
    }
    frameInitCommandBufferSubmissions.push_back(frameInitCommandBufferSubmitInfo);
    ...
    VkSubmitInfo2 frameInitSubmitInfo{
        ...
        .commandBufferInfoCount = static_cast<uint32_t>(frameInitCommandBufferSubmissions.size()),
        .pCommandBufferInfos = frameInitCommandBufferSubmissions.data(),
...
void vulkan::VulkanRenderer::allocateAndRecordFrameInitCommandBuffer(VkCommandBuffer &frameInitCommandBuffer) {
