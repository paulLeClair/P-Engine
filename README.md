...
Since it's a sort of personal active learning tool at this point, lots of it is still only sketchwork! I haven't been able to work on lots of important pieces yet, since game engines are very complicated. Most functioning game engines are developed by large teams of ultra-geniuses, and so I'm trying to keep the scope limited so as to be within my own reach.
...
It may not be required to use these at all, and you can attach code to the engine yourself within an `EngineMode` subclass, but this is a way to separate things out cleanly and have the user's `EngineMode` simply refer to the `EngineTool` subclass instead of having to implement the `EngineTool` subclass functionality itself, and it can be easily passed to other modes that want to make use of it.
...
To describe the rendering processes they're using, the user specifies the passes and subpasses they need, as well as the rendering resources to be used (apart from geometry). The idea is to do this while entering a particular engine mode, and then the graph will be able to render frames after it is baked. Since we're directly mapping each `Pass` into a sequence of `VkRenderPass`es plus everything required to submit some specified commands for execution, it should be fairly general-purpose. Ideally it should handle many of Vulkan's use cases especially as I continue to fill in missing features.
...
Often you're going to be dealing with vast arrays of geometry data, and so I want to make it easy to break it up logically and feed it into the engine. The `Scene` will accept some set of `Renderable`s, which contain a higher-level representation of the geometry. The user-supplied data can take many forms, and various vertex attributes are able to be enabled with a bitmask. The ordering of the attributes (in memory) is hardcoded, so you have to package your vertex data in the order the engine expects.
...
Right now the `Scene` is implemented in a kind of Struct-of-Arrays form, but I may rewrite it or otherwise integrate some sort of **scene graph** functionality, which is a common technique used in game engines to break a scene into its components. In both cases, it's fairly simple to connect the constituent renderables to a particular subpass in the scene's render graph, so I hope that I can provide a hybrid interface that allows both.
...
- the `VulkanGUIHandler` simply manages all Vulkan state for the DearIMGUI library integration
