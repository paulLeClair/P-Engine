...
namespace pEngine::girEngine::backend::appContext::vulkan {
    ...
    const std::unordered_map<VulkanLogicalDevice::SupportedDeviceLayer, std::string> VulkanLogicalDevice::DEVICE_LAYER_NAME_CONVERSION_MAP = {
            // TODO - add in the conversions to name strings that will be fed to vulkan
    };


    const std::unordered_map<VulkanLogicalDevice::SupportedDeviceExtension, std::string> VulkanLogicalDevice::DEVICE_EXTENSION_NAME_CONVERSION_MAP = {
            // TODO - add in the conversions to name strings that will be fed to vulkan
            {VulkanLogicalDevice::SupportedDeviceExtension::SWAPCHAIN_EXTENSION, VK_KHR_SWAPCHAIN_EXTENSION_NAME},
            {VulkanLogicalDevice::SupportedDeviceExtension::SYNC_2,              VK_KHR_SYNCHRONIZATION_2_EXTENSION_NAME}
    };
    ...
    VulkanLogicalDevice::VulkanLogicalDevice(const VulkanLogicalDevice::CreationInput &creationInput) {
        ...
        if (std::any_of(enabledDeviceExtensions.begin(), enabledDeviceExtensions.end(), [&](const char *extensionName) {
